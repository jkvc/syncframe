...
/* eslint-env node */
/**
 * Smoke test dot demo API against a running dev server.
 * Usage: node scripts/test-dot-api.mjs [baseUrl]
 * Default baseUrl: http://localhost:3000
 */

const BASE = process.argv[2] ?? 'http://localhost:3000';
const API = `${BASE}/api/dot`;
...
async function main() {
  ...
  r = await json('POST', '/control', { action: 'start' });
  ...
  console.log('OK — dot API smoke test passed');
}
